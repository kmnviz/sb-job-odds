...
export async function pollOdds() {
    ...
    const marketType = 'asian_handicap';
    const now = new Date();
    ...
    const [market, bookmaker] = await Promise.all([
        db.collection('markets').findOne({ type: marketType }),
        db.collection('bookmakers').findOne({ name: bookmakerName }),
    ]);
    if (!market) {
    ...
    let errors = 0;
    const fetchedByFixture = new Map();
    try {
    ...
    for (const match of matches) {
        ...
        for (const odd of odds) {
            rowsToInsert.push(buildOddsSnapshot({
                fetchedOdd: odd,
                matchId: match._id,
                marketId: market._id,
                marketType,
...
//# sourceMappingURL=poll-odds.service.js.map